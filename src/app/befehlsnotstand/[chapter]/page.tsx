...
function audioSrc(slug: string): string | null {
  const file = path.join(process.cwd(), "public", "book", "audio-de", `${slug}.mp3`);
  return fs.existsSync(file) ? `/book/audio-de/${slug}.mp3` : null;
}

export default async function ChapterPageDe({
  params,
}: {
  params: Promise<{ chapter: string }>;
}) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-8">
        ...
        <span>/</span>
        <Link href="/befehlsnotstand" className="hover:text-white transition-colors">
          Befehlsnotstand
        </Link>
        <span>/</span>
      ...
      <article className="max-w-2xl mx-auto">
        <p className="text-xs uppercase tracking-widest text-gray-500 mb-3">
          Teil {ch.part} — {parts[String(ch.part)]}
        </p>
        ...
        <nav className="flex items-stretch justify-between gap-4 mt-16 pt-8 border-t border-white/10">
          {prev ? (
            <Link
              href={`/befehlsnotstand/${prev.slug}`}
          ...
          {next ? (
            <Link
              href={`/befehlsnotstand/${next.slug}`}
        ...
        <p className="text-xs text-gray-600 mt-8">
          ...
          >
            Dieses Kapitel auf Englisch lesen →
          </Link>
        </p>
      </article>
    </div>
